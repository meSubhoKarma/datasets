...
def _generate_stl10_data():
  ...
  for num_examples, fname in enumerate(["train_y.bin", "test_y.bin"]):
    labels = np.random.randint(
        NUMBER_LABELS, size=(num_examples + 1), dtype=np.uint8)
    dump(stl_output_dir(), fname, labels)

  for num_examples, fname in enumerate(
      ["train_X.bin", "test_X.bin", "unlabeled_X.bin"]):
    images = np.random.randint(
        256, size=(num_examples + 1, HEIGHT * WIDTH * 3), dtype=np.uint8)
