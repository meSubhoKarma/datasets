...
class Stl10(tfds.core.GeneratorBasedBuilder):
  ...
  def _stl_info(self):
    return STLInfo(
        ...
        prefix="stl10_binary/",
        label_files=["class_names.txt"],
        label_keys=["label"],
    )

  def _split_generators(self, dl_manager):
    """Returns SplitGenerators."""
    stl10_path = dl_manager.download_and_extract(self._stl_info.url)
    stl10_info = self._stl_info

    stl10_path = os.path.join(stl10_path, stl10_info.prefix)
    ...
    splits = [
        tfds.core.SplitGenerator(
            ...
            gen_kwargs={"filepaths": gen_filenames(stl10_info.train_files)}),
        tfds.core.SplitGenerator(
            name=UNLABELLED,
            gen_kwargs={"filepaths": gen_filenames(stl10_info.unlabeled_files)
                       }),
        tfds.core.SplitGenerator(
            name=tfds.Split.TEST,
            gen_kwargs={"filepaths": gen_filenames(stl10_info.test_files)}),
    ]
    return splits
  ...
  def _generate_examples(self, filepaths):
    ...
    """
    label_key = self._stl_info.label_keys[0]
    ...
    for index, image in enumerate(images):
      ...
      if labels is None:
        ...
        record[label_key] = labels[index]
      yield index, record


class STLInfo(
    collections.namedtuple("_STLInfo", [
        "name",
        "url",
        "prefix",
        "train_files",
  ...
  """Contains the information necessary to generate a STL-10 dataset.
  ...
  Attributes:
    ...
    unlabeled_files  (list<str>): name of unlabelled files within `prefix`.
